...
def requested(value, known):
    """Ticked families of the checkbox group -> family ids, in form order.

    GitHub renders the group as `- [X] Na. Title` / `- [ ] Na. Title` lines, so
    only the ticked lines count. A plain comma-separated selection (the old
    dropdown form) is accepted too; family titles contain commas, so the `Na.`
    prefixes are the delimiters in both cases."""
    if "- [" in value:
        value = "\n".join(l for l in value.splitlines() if re.match(r"\s*- \[[xX]\]", l))
    ids = []
...
def self_test():
    body = ("### Abbreviation\n\nZZProbe\n\n### Paper title\n\nAn Image is Worth 1/2 Tokens\n\n"
            ...
            "### Weights link\n\n_No response_\n\n"
            "### Where does it belong?\n\n- [X] 1a. Temporal sampling and selection\n"
            "- [x] 1b. Patch, resolution, and input-layout budgeting\n"
    ...
    assert requested("3a. Selection and merging, 4a. Decoder token pruning", known) == ["3a", "4a"]
    row = build_row(f)
